...
function resolveCodeAction(ctx) {
    ...
    return async (params) => {
        const edit = await client.sendRequest(ra.resolveCodeAction, params);
        if (!edit)
            return;
        await applySnippetWorkspaceEdit(edit);
    };
...
class Ctx {
    ...
    async startServer() {
        ...
        client.onNotification(ra.status, async (status) => {
            ...
            if (status === 'ready') {
                ...
                const prompt = await coc_nvim_1.workspace.showPrompt(`rust-analyzer needs to reload project`);
                if (prompt) {
    ...
    async checkUpdate(auto = true) {
        ...
        const ret = await coc_nvim_1.workspace.showQuickpick(['Yes', 'Check GitHub releases', 'Cancel'], msg);
        if (ret === 0) {
            try {
...
exports.memoryUsage = new lc.RequestType('rust-analyzer/memoryUsage');
exports.status = new lc.NotificationType('rust-analyzer/status');
exports.reloadWorkspace = new lc.RequestType('rust-analyzer/reloadWorkspace');
...
Config.requiresReloadOpts = ['serverPath', 'cargo', 'procMacro', 'files', 'updates', 'lens'].map((opt) => `${Config.rootSection}.${opt}`);
